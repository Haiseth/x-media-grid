...
(function () {
  ...
  const I18N_TABLES = {
    "ja": {
      ...
      "optAboutIntro": "X（旧Twitter）のメディア/いいね/ブックマーク/ホームを、画像がずらっと並ぶグリッド表示に変えて、キーボードだけでサクサク見て回れるようにする拡張機能です。いいね・ブックマーク・リポスト・リプライも1キーで行えます。以下、主な機能を重要な順に紹介します。",
      "optFeat1H": "1. グリッド表示",
    ...
    "en": {
      ...
      "optAboutIntro": "This extension turns X (formerly Twitter) media, likes, bookmarks and Home into a grid of images you can browse quickly with the keyboard alone. Liking, bookmarking, reposting and replying each take a single key. The main features, in order of importance:",
  ...
  function harvestNew() {
    ...
    for (const c of Array.from(Grid.sourceRoot.children)) {
      ...
      if (photoImgs.length === 0) {
        ...
        // 「本当に画像なし」と確定してxmrDoneを付け、そのセルを見なくなる。
        // ところがXは画面から遠いセルの画像を読み込まないため、収集スクロールで
        // 通り過ぎただけの投稿が「テキスト」と誤判定される（実測：写真リンクは
        // 在るのにtweetPhotoが0個）。
        // ※完了印はセル要素に付き、Xはセルを使い回すので印ごと入れ替わる。
        //   よって永久に失われるわけではなく、もう一度その辺りを通れば拾える
        //   （実機で再スクロールすると出てくることを確認）。とはいえ
        //   「1回目で出ない」時点で取りこぼしには違いないので、下の対策は残す。
